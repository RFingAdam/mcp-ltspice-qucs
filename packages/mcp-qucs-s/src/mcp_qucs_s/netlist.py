...
def generate_coupled_array_netlist(
    ...
    *,
    ...
    """Write the exact ideal-TEM graph netlist of a coupled-line array
    (interdigital or combline).
    ...
    :func:`mcp_qucs_s.distributed.interdigital_bpf` or
    :func:`mcp_qucs_s.distributed.combline_bpf` (``y_c``,
    ``segments_deg``, ``bottom``, ``top``, ``ports``, ``f0_hz``; a
    combline design additionally carries ``cap_loads`` — (level, line,
    farad) lumped loading capacitors emitted as ideal ``C`` elements to
    ground).
    """
    import numpy as np
    ...
    for idx, (level, line, farad) in enumerate(design.get("cap_loads") or [], start=1):
        body.append(f'C:CL{idx} {node(level, line)} {GROUND} C="{_fmt(farad)}"')

    port_z = f'Z="{_fmt(z0)} Ohm"'
    header = [
        f"# {title}",
        f"# coupled-array graph netlist: {n_lines} lines, {n_seg} segments",
    ]
...
#: The interdigital name predates combline support; both emit the same
#: exact graph netlist.
generate_interdigital_netlist = generate_coupled_array_netlist
